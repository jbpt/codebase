...
public class EventLogSampling extends EventLogUtils {
	...
	public static EventLog logSamplingWithReplacement(EventLog log, int n) {
		if (log == null || log.isEmpty() || n <= 0) return null;
		
		EventLog result = new EventLog();
		
		for (int i=0; i<n; i++) {
			result.add(random(log));
		}
	...
	private static List<BSite> getBreedingSites(Trace t1, Trace t2, int k) {
		...
		if (t1.size() < k || t2.size() < k) return null; // early exit
		
		List<BSite> result = new ArrayList<>();
		for (int p1 = 0; p1 <= t1.size() - k; p1++) {
			for (int p2 = 0; p2 <= t2.size() - k; p2++) {
				boolean isCommonSubtraceExists = true;
				
				for (int i = 0; i < k; i++) {
					if (!t1.get(p1 + i).equals(t2.get(p2 + i))) {
						isCommonSubtraceExists = false;
						break;
					}
				}
				
				if (isCommonSubtraceExists) {
	...
	private static EventLog logBreeding(EventLog log1, EventLog log2, int k, double p) {
		...
		EventLog result = new EventLog();
		int n = (log1.size() + 1) / 2; // (log1.size() % 2 == 0) ? log1.size() / 2 : (log1.size() + 1) / 2;

		for (int i = 0; i < n; i++) {
			Trace t1 = random(log1);
			Trace t2 = random(log2);
			Collection<BSite> sites = getBreedingSites(t1, t2, k); // algorithm 3

			if (new Random().nextDouble() < p && !sites.isEmpty()) { // added the functionality of breeding probability
				BSite site = random(sites);
				result.add(Trace.crossover(t1, t2, site, k));
				result.add(Trace.crossover(t2, t1, new BSite(site.p2, site.p1), k));
			} else {
				result.add(t1);
				result.add(t2);
			}
	...
	public static EventLog logSamplingWithBreeding(EventLog log, int n, int g, int k, double p) {
		...
		generations[0] = log;
		for (int i = 1; i <= g; i++) {
			generations[i] = logBreeding(log, generations[i-1], k, p);
		}
		
		EventLog LL = new EventLog();
		for (EventLog bredLog : generations) {
			if (bredLog != null) LL.addAll(bredLog);
		}
		
		return EventLogSampling.logSamplingWithReplacement(LL, n);
	}
