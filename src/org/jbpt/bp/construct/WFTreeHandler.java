...
public class WFTreeHandler {
	...
	public WFTreeHandler(IPetriNet<Flow,Node,Place,Transition> net) {
	...
	private void orderPNodes(RPSTNode<Flow, Node> node) {
		if (node.getType() == TCType.POLYGON) {
			Vector<RPSTNode<Flow, Node>> orderedChildren = new Vector<>();
			Collection<RPSTNode<Flow, Node>> childrenCopy = new ArrayList<>(this.wfTree.getChildren(node));
		...
		// call recursively
		for (RPSTNode<Flow, Node> child: this.wfTree.getChildren(node))
			orderPNodes(child);
	}
