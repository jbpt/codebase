...
public class Unfolding {
	...
	private Map<Condition,Set<Cut>> c2cut = new HashMap<Condition,Set<Cut>>();
	...
	protected Map<Transition,Set<Cut>> t2cuts	= new HashMap<Transition,Set<Cut>>();
	...
	protected Collection<Event> getPossibleExtensions() {
		...
		for (Transition t : this.net.getTransitions()) {
			if (t2cuts.get(t)==null) continue;

			for (Cut cut : t2cuts.get(t)) {
				Coset coset = this.containsPlaces(cut,this.net.getPreset(t)); 
				boolean flag = false;
				if (t2es.get(t)!=null) {
	...
	protected Set<Cut> getCutsWithPlace(Place p) {
		...
		Set<Condition> cs = p2cs.get(p);
		if (cs==null) return result;
		for (Condition c : cs) {
			Set<Cut> cuts = c2cut.get(c);
	...
	protected boolean addEvent(Event e) {		
		...
		for (Cut cut : t2cuts.get(e.getTransition())) {
			Coset coset = this.containsPlaces(cut, this.net.getPreset(e.getTransition()));
			if (coset!=null) {
				cuts.add(cut);
			}
		}
		t2cuts.get(e.getTransition()).removeAll(cuts);*/
		// END TODO
	...
	protected boolean addCut(Cut cut) {
		...
		for (Transition t : this.net.getTransitions()) {
			Coset coset = this.containsPlaces(cut,this.net.getPreset(t));
			if (coset != null) {
				if (t2cuts.get(t) == null) {
					Set<Cut> cuts = new HashSet<Cut>();
					cuts.add(cut);
					t2cuts.put(t, cuts);
				}
				else
					t2cuts.get(t).add(cut);
			}
		}
		// END TODO
